...
export function KTXDomA() {
    ...
    useEffect(() => {
        fetch("https://backend-mapdevice.onrender.com/api/ktxdoma")
    ...
    const handleDelete = (id) => {
        ...
        if (confirmDelete) {
            fetch(`https://backend-mapdevice.onrender.com/api/ktxdoma/${id}`, {
                method: "DELETE",
            })
                .then(async (res) => {
                    if (!res.ok) {
    ...
    const handleSaveName = (wifiId) => {
        ...
        fetch(`https://backend-mapdevice.onrender.com/api/ktxdoma/${wifiId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
    ...
    return (
        <div className="ktx">
            <div className="map-container-ktx" onClick={handleMapClick} ref={mapRef}>
                <img src={image} alt="KTX Dom A" className="map-image-ktx" />
