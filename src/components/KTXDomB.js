...
export function KTXDomB() {
    ...
    useEffect(() => {
        fetch("https://backend-mapdevice.onrender.com/api/ktxdomb")
    ...
    const handleDelete = (id) => {
        ...
        if (confirmDelete) {
            fetch(`https://backend-mapdevice.onrender.com/api/ktxdomb/${id}`, {
                method: "DELETE",
            })
                .then(async (res) => {
                    if (!res.ok) {
    ...
    const handleSaveName = (wifiId) => {
        ...
        fetch(`https://backend-mapdevice.onrender.com/api/ktxdomb/${wifiId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
